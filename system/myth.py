...
from discord import Intents, Message, Embed, CustomActivity
...
class Myth(AutoShardedBot):
    ...
    async def on_message(self, message: Message):
        ...
        if len(self.message_cache[author_id]) >= 10:
            ...
            embed = Embed(description=f"> {emoji.deny} {message.author.mention}: **You got blacklisted,** if you think is by accident join the [support server](https://discord.gg/uid)", color=color.deny)
