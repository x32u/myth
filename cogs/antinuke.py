...
class AntiNuke(commands.Cog):
    ...
    async def logs(self, guild_id: int, user_id: int, action_type: str, details: str):
        try:
            ...
            await self.client.pool.execute("""
                ...
                VALUES ($1, $2, $3, $4, $5)
            """, guild_id, user_id, action_type, datetime.utcnow(), details)
            ...
            if settings.get('log_channel'):
                ...
                if guild:
                    ...
                    if log_channel:
                        ...
                        user_pfp = punished_user.avatar.url if punished_user.avatar else punished_user.default_avatar.url
                        ...
                        embed.set_author(name=f"{punished_user.name} | antinuke", icon_url=user_pfp)
                        embed.add_field(name="<:29:1298731238655266847> Action", value=f"> `{punishment}`", inline=True)
                        embed.add_field(name="<:18:1298392181169721468> User", value=f"> <@{user_id}> ({user_id})", inline=True)
                        embed.add_field(name="", value=f"‎ ‎ ", inline=True)
                        embed.add_field(name="<:28:1298731241209729024> Details", value=f"> {details}", inline=True)
                        embed.add_field(name="<:22:1298731253473874003> Time", value=f"> `{time_taken:.2f}s`", inline=True)
    ...
    async def threshold(self, guild_id: int, user_id: int, action_type: str) -> bool:
        ...
        self.recent_actions[key] = [
            time for time in self.recent_actions[key]
    ...
    @commands.group(
        description="Protect your server agaisnt mad kids", 
        aliases=["an"]
    )
    ...
    @commands.has_permissions(administrator=True)
    async def antinuke_channeldelete(self, ctx, option: str, punishment: Optional[str] = None, threshold: Optional[int] = None):
        ...
        if option == "on":
            ...
            await self.client.pool.execute("UPDATE antinuke SET channeldelete = $1, punishment = $2 WHERE guild_id = $3", threshold, punishment.lower(), ctx.guild.id)
            embed = discord.Embed(description=f"> <:28:1298731241209729024> **Option:** `enabled` \n> <:29:1298731238655266847> **Punishment:** `{punishment.lower()}` \n> <:22:1298731253473874003> **Threshold:** {threshold}", color=color.default)
            user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
            embed.set_author(name=f"{ctx.author.name} | Enabled channel delete", icon_url=user_pfp)
            await ctx.send(embed=embed)
        else:
            await self.client.pool.execute("UPDATE antinuke SET channeldelete = NULL WHERE guild_id = $1", ctx.guild.id)
    ...
    @commands.has_permissions(administrator=True)
    async def antinuke_channelcreate(self, ctx, option: str, punishment: Optional[str] = None, threshold: Optional[int] = None):
        if not await self.is_admin(ctx.guild.id, ctx.author.id):
            await ctx.deny("**You're** not an antinuke admin")
            return
        ...
        if option == "on":
            ...
            await self.client.pool.execute("UPDATE antinuke SET channelcreate = $1, punishment = $2 WHERE guild_id = $3", threshold, punishment.lower(), ctx.guild.id)
            embed = discord.Embed(description=f"> <:28:1298731241209729024> **Option:** `enabled` \n> <:29:1298731238655266847> **Punishment:** `{punishment.lower()}` \n> <:22:1298731253473874003> **Threshold:** {threshold}", color=color.default)
            user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
            embed.set_author(name=f"{ctx.author.name} | Enabled channel create", icon_url=user_pfp)
            await ctx.send(embed=embed)
        else:
            await self.client.pool.execute("UPDATE antinuke SET channelcreate = NULL WHERE guild_id = $1", ctx.guild.id)
    ...
    @commands.has_permissions(administrator=True)
    async def antinuke_roledelete(self, ctx, option: str, punishment: Optional[str] = None, threshold: Optional[int] = None):
        if not await self.is_admin(ctx.guild.id, ctx.author.id):
            await ctx.deny("**You're** not an antinuke admin")
            return
        ...
        if option == "on":
            ...
            await self.client.pool.execute("UPDATE antinuke SET roledelete = $1, punishment = $2 WHERE guild_id = $3", threshold, punishment.lower(), ctx.guild.id)
            embed = discord.Embed(description=f"> <:28:1298731241209729024> **Option:** `enabled` \n> <:29:1298731238655266847> **Punishment:** `{punishment.lower()}` \n> <:22:1298731253473874003> **Threshold:** {threshold}", color=color.default)
            user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
            embed.set_author(name=f"{ctx.author.name} | Enabled role delete", icon_url=user_pfp)
            await ctx.send(embed=embed)
        else:
            await self.client.pool.execute("UPDATE antinuke SET roledelete = NULL WHERE guild_id = $1", ctx.guild.id)
    ...
    @commands.has_permissions(administrator=True)
    async def antinuke_rolecreate(self, ctx, option: str, punishment: Optional[str] = None, threshold: Optional[int] = None):
        if not await self.is_admin(ctx.guild.id, ctx.author.id):
            await ctx.deny("**You're** not an antinuke admin")
            return
        ...
        if option == "on":
            ...
            await self.client.pool.execute("UPDATE antinuke SET rolecreate = $1, punishment = $2 WHERE guild_id = $3", threshold, punishment.lower(), ctx.guild.id)
            embed = discord.Embed(description=f"> <:28:1298731241209729024> **Option:** `enabled` \n> <:29:1298731238655266847> **Punishment:** `{punishment.lower()}` \n> <:22:1298731253473874003> **Threshold:** {threshold}", color=color.default)
            user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
            embed.set_author(name=f"{ctx.author.name} | Enabled role create", icon_url=user_pfp)
            await ctx.send(embed=embed)
        else:
            await self.client.pool.execute("UPDATE antinuke SET rolecreate = NULL WHERE guild_id = $1", ctx.guild.id)
    ...
    @commands.has_permissions(administrator=True)
    async def antinuke_roleupdate(self, ctx, option: str, punishment: Optional[str] = None, threshold: Optional[int] = None):
        if not await self.is_admin(ctx.guild.id, ctx.author.id):
            await ctx.deny("**You're** not an antinuke admin")
            return
        ...
        if option == "on":
            ...
            await self.client.pool.execute("UPDATE antinuke SET roleupdate = $1, punishment = $2 WHERE guild_id = $3", threshold, punishment.lower(), ctx.guild.id)
            embed = discord.Embed(description=f"> <:28:1298731241209729024> **Option:** `enabled` \n> <:29:1298731238655266847> **Punishment:** `{punishment.lower()}` \n> <:22:1298731253473874003> **Threshold:** {threshold}", color=color.default)
            user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
            embed.set_author(name=f"{ctx.author.name} | Enabled role update", icon_url=user_pfp)
            await ctx.send(embed=embed)
        else:
            await self.client.pool.execute("UPDATE antinuke SET roleupdate = NULL WHERE guild_id = $1", ctx.guild.id)
    ...
    @commands.has_permissions(administrator=True)
    async def antinuke_webhookcreate(self, ctx, option: str, punishment: Optional[str] = None, threshold: Optional[int] = None):
        if not await self.is_admin(ctx.guild.id, ctx.author.id):
            await ctx.deny("**You're** not an antinuke admin")
            return
        ...
        if option == "on":
            ...
            await self.client.pool.execute("UPDATE antinuke SET webhookcreate = $1, punishment = $2 WHERE guild_id = $3", threshold, punishment.lower(), ctx.guild.id)
            embed = discord.Embed(description=f"> <:28:1298731241209729024> **Option:** `enabled` \n> <:29:1298731238655266847> **Punishment:** `{punishment.lower()}` \n> <:22:1298731253473874003> **Threshold:** {threshold}", color=color.default)
            user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
            embed.set_author(name=f"{ctx.author.name} | Enabled webhook create", icon_url=user_pfp)
            await ctx.send(embed=embed)
        else:
            await self.client.pool.execute("UPDATE antinuke SET webhookcreate = NULL WHERE guild_id = $1", ctx.guild.id)
            await ctx.agree("**Disabled** webhook create")

    @antinuke.command(
        name="ban",
        description="Punish someone for banning to many users"
    )
    @commands.has_permissions(administrator=True)
    async def antinuke_ban(self, ctx, option: str, punishment: Optional[str] = None, threshold: Optional[int] = None):
        if not await self.is_admin(ctx.guild.id, ctx.author.id):
            await ctx.deny("**You're** not an antinuke admin")
            return
        ...
        if option == "on":
            ...
            await self.client.pool.execute("UPDATE antinuke SET ban = $1, punishment = $2 WHERE guild_id = $3", threshold, punishment.lower(), ctx.guild.id)
            embed = discord.Embed(description=f"> <:28:1298731241209729024> **Option:** `enabled` \n> <:29:1298731238655266847> **Punishment:** `{punishment.lower()}` \n> <:22:1298731253473874003> **Threshold:** {threshold}", color=color.default)
            user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
            embed.set_author(name=f"{ctx.author.name} | Enabled ban", icon_url=user_pfp)
        else:
            await self.client.pool.execute("UPDATE antinuke SET ban = NULL WHERE guild_id = $1", ctx.guild.id)
            await ctx.agree("**Disabled** ban")

    @antinuke.command(
        name="kick",
        description="Punish someone for kicking to many users"
    )
    @commands.has_permissions(administrator=True)
    async def antinuke_kick(self, ctx, option: str, punishment: Optional[str] = None, threshold: Optional[int] = None):
        if not await self.is_admin(ctx.guild.id, ctx.author.id):
            await ctx.deny("**You're** not an antinuke admin")
            return
        ...
        if option == "on":
            ...
            await self.client.pool.execute("UPDATE antinuke SET kick = $1, punishment = $2 WHERE guild_id = $3", threshold, punishment.lower(), ctx.guild.id)
            embed = discord.Embed(description=f"> <:28:1298731241209729024> **Option:** `enabled` \n> <:29:1298731238655266847> **Punishment:** `{punishment.lower()}` \n> <:22:1298731253473874003> **Threshold:** {threshold}", color=color.default)
            user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
            embed.set_author(name=f"{ctx.author.name} | Enabled kick", icon_url=user_pfp)
            await ctx.send(embed=embed)
        else:
            await self.client.pool.execute("UPDATE antinuke SET kick = NULL WHERE guild_id = $1", ctx.guild.id)
            await ctx.agree("**Disabled** kick")

    @antinuke.command(
        name="logs",
        description="Send logs into a channel"
    )
    @commands.has_permissions(administrator=True)
    async def antinuke_logs(self, ctx, channel: discord.TextChannel):
        ...
        if settings:
            ...
            await self.client.pool.execute("INSERT INTO antinuke (guild_id, log) VALUES ($1, $2)", guild_id, channel_id)
    ...
    async def admin(self, ctx, option: str, user: discord.Member):
        ...
        if option == "add":
            await self.client.pool.execute("INSERT INTO antinuke_admins (guild_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", guild_id, user_id)
            await ctx.agree(f"**Added** {user.mention} as an antinuke admin")
        elif option == "remove":
            await self.client.pool.execute("DELETE FROM antinuke_admins WHERE guild_id = $1 AND user_id = $2", guild_id, user_id)
