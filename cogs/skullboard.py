    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if str(reaction.emoji) == emoji:
            ...
            if reaction_total >= reaction_count:
                ...
                embed = discord.Embed(description=f'### **[#{reaction.message.channel.name}]({reaction.message.jump_url})**\n {reaction.message.content}', color=color.default)
                user_pfp = reaction.message.author.avatar.url if reaction.message.author.avatar else reaction.message.author.default_avatar.url
                embed.set_author(name=reaction.message.author.name, icon_url=user_pfp)

                if reaction.message.reference:
                    ref_message = await reaction.message.channel.fetch_message(reaction.message.reference.message_id)
                    embed.add_field(name='', value=f'**[Replying]({ref_message.jump_url})** to {reaction.message.author.mention}', inline=False)
                ...
                if message_id in self.skull_messages:
                    ...
                    await sent_message.edit(content=f"**#{reaction_total}** {emoji}", embed=embed)
                else:
                    sent_message = await channel.send(content=f"**#{reaction_total}** {emoji}", embed=embed)
        ...
        if str(reaction.emoji) == emoji:
            ...
            if message_id in self.user_reactions:
                if user_id in self.user_reactions[message_id]:
                    ...
                    if message_id in self.skull_messages:
                        sent_message = self.skull_messages[message_id]
                        await sent_message.edit(content=f"**#{reaction_total}** {emoji}")
