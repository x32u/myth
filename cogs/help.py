...
class HelpDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user != self.author:
            embed = discord.Embed(description=f"> {emoji.warn} {self.author.mention}: You **cannot** interact with this", color=color.warn)
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return

        selected_cog = self.values[0]
        if selected_cog == "Home":
            await interaction.response.edit_message(content="", embed=self.home, view=self.view)
            return

        cog = self.client.get_cog(selected_cog)
        commands_list = [
            f"{command.name}*" if isinstance(command, commands.Group) else f"{command.name}"
        ...
        bot_pfp = self.client.user.avatar.url if self.client.user.avatar else self.client.user.default_avatar.url
