...
class Vanityroles(commands.Cog):
    ...
    @commands.group(
        description="Give out roles on a specific presence", 
        aliases=["vanity", "vr"]
    )
    ...
    @vanityroles.command(
        name="set",
        description="Set on/off the vanityroles"
    )
    async def vanityroles_set(self, ctx, status: str):
        if status.lower() == "on":
            ...
            await self.client.pool.execute("UPDATE vanityroles SET enabled = FALSE WHERE guild_id = $1", ctx.guild.id)
