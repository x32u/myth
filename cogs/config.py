...
class Config(commands.Cog):
    ...
    @pingonjoin.command(
        name="add", 
        description="Add a channel to pingonjoin"
    )
    @commands.has_permissions(manage_channels=True)
    ...
    @pingonjoin.command(
        name="remove", 
        description="Remove a channel from pingonjoin"
    )
    @commands.has_permissions(manage_channels=True)
    ...
    @pingonjoin.command(
        name="list", 
        description="Check the pingonjoin channel list"
    )
    @commands.has_permissions(manage_channels=True)
    async def pingonjoin_list(self, ctx):
        ...
        channel_mentions = '\n> '.join([f"<#{record['channel_id']}>" for record in channels])
        user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
    ...
    @pingonjoin.command(
        name="clear", 
        description="Clear all pingonjoin settings"
    )
    @commands.has_permissions(manage_channels=True)
    ...
    @commands.group(
        description="Respond to certain messages", 
        aliases=["autoresp", "ap"]
    )
    ...
    @autorespond.command(
        name="add", 
        description="Add a response to a trigger"
    )
    @commands.has_permissions(manage_messages=True)
    ...
    @autorespond.command(
        name="remove", 
        description="Remove a response from a trigger"
    )
    @commands.has_permissions(manage_messages=True)
    ...
    @autorespond.command(
        name="list", 
        description="Check the autorespond list"
    )
    @commands.has_permissions(manage_messages=True)
    async def autorespond_list(self, ctx):
        ...
        embed.set_author(name=f"{ctx.author.name} | Autorespond List", icon_url=user_pfp)

        for record in responses:
            embed.add_field(name=record['trigger'], value=f"> {record['response']}", inline=False)
        
        await ctx.send(embed=embed)

    @autorespond.command(
        name="clear", 
        description="Clear all autorespond settings"
    )
    @commands.has_permissions(manage_messages=True)
    ...
    @commands.group(
        description="React to certain messages with emojis", 
        aliases=["autorea", "ac"]
    )
    @commands.has_permissions(manage_messages=True)
    ...
    @autoreact.command(
        name="remove", 
        description="Remove a reaction from a trigger"
    )
    @commands.has_permissions(manage_messages=True)
    ...
    @autoreact.command(
        name="list", 
        description="Check the autoreact list"
    )
    @commands.has_permissions(manage_messages=True)
    async def autoreact_list(self, ctx):
        ...
        for record in reacts:
            embed.add_field(name=record['trigger'], value=f"> {record['emoji']}", inline=False)
        
        await ctx.send(embed=embed)

    @autoreact.command(
        name="clear", 
        description="Clear all autoreact settings"
    )
    @commands.has_permissions(manage_messages=True)
    ...
    @commands.group(
        description="Change messages for multiple commands"
    )
    ...
    @invoke.command(
        name="ban", 
        description="Change the response from ban"
    )
    @commands.has_permissions(manage_guild=True)
    ...
    @invoke.command(
        name="softban", 
        description="Change the response from softban"
    )
    @commands.has_permissions(manage_guild=True)
    ...
    @invoke.command(
        name="unban", 
        description="Change the response from unban"
    )
    @commands.has_permissions(manage_guild=True)
    ...
    @invoke.command(
        name="kick", 
        description="Change the response from kick"
    )
    @commands.has_permissions(manage_guild=True)
    ...
    @invoke.command(
        name="clear",
        description="Clear all invoke settings"
    )
    @commands.has_permissions(manage_guild=True)
    ...
    @commands.group(
        description="DM people when they join"
    )
    @commands.has_permissions(manage_guild=True)
    ...
    @joindm.command(
        name="message", 
        description="Set the joindm message"
    )
    ...
    async def reactionroles_add(self, ctx, role: discord.Role = None, message: discord.Message = None, emoji: str = None):
        if role is None:
            await ctx.warn("**Mention** a role")
            return
    ...
    @commands.has_permissions(manage_roles=True)
    async def reactionroles_remove(self, ctx, role: discord.Role = None, message: discord.Message = None, emoji: str = None):
        if role is None:
            await ctx.warn("**Mention** a role")
            return
        ...
        if not before.bot and str(before) != str(after):
            ...
            for row in channel_ids:
                ...
                if channel := self.client.get_channel(channel_id):
                    ...
                    await channel.send(f"The username **{before}** is available {formatted_date}")
    ...
    async def on_guild_update(self, before, after):
        if before.vanity_url_code and before.vanity_url_code != after.vanity_url_code:
            ...
            for row in channel_ids:
                ...
                if channel := self.client.get_channel(channel_id):
                    await asyncio.sleep(0.1)
                    await channel.send(f"The vanity **/{before.vanity_url_code}** is available")
